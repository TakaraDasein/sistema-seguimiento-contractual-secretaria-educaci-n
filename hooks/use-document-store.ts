...
export function useDocumentStore() {
  ...
  useEffect(() => {
    const loadData = () => {
      try {
        const storedFolders = localStorage.getItem("proveedores-folders")
        const storedDocuments = localStorage.getItem("proveedores-documents")
  ...
  useEffect(() => {
    if (!isLoading) {
      localStorage.setItem("proveedores-folders", JSON.stringify(folders))
    }
  }, [folders, isLoading])

  useEffect(() => {
    if (!isLoading) {
      localStorage.setItem("proveedores-documents", JSON.stringify(documents))
    }
  }, [documents, isLoading])
